...
import type { AuthCardOptions } from "../types/app/authType";
...
export default class AuthCard {
  ...
  constructor(options: AuthCardOptions) {
    ...
    this.logoIcon =
      this.mode === "register" ? new RegistrationLogo() : new LoginLogo();
    this.logoEl = el("div.auth-card__logo", this.logoIcon) as HTMLDivElement;
    ...
    this.titleEl = el("p.auth-card__title", titleText) as HTMLParagraphElement;
    ...
    const authText = this.mode === "register" ? "Регистрация" : "Войти";
  ...
  private renderInputError(inputEl: HTMLInputElement) {
      inputEl.classList.toggle('custom-input__field--error')
      setTimeout(() => {
        inputEl.classList.toggle('custom-input__field--error')
      }, 3000);
      return
  ...
  private handleAuthClick = (e: MouseEvent) => {
    ...
    if (!username) return this.renderInputError(this.loginInput.inputEl)
    if (!password) return this.renderInputError(this.loginInput.inputEl)
  ...
  private async login(username: string, password: string) {
    try {
      ...
      if (res && typeof res !== "string") {
        ...
        const newEl = el("div.auth-card", String(res)) as HTMLDivElement;
